...
def main() -> None:
    ...
    with open("../input.txt", "r") as file:
        ...
        time = int(''.join([time for time in lines[0][len('Time:      '):].split(' ') if time != '']))
        # Line 2 is distances
        distance = int(''.join([distance for distance in lines[1][len('Distance:  '):].split(' ') if distance != '']))

        print(f"Total = {winPossibilities(time, distance)}")
